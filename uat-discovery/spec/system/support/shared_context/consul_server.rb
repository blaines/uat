require 'json'
require 'webmock/rspec'

RSpec.shared_context 'real or mock discovery server based on environment' do
  before(:each) do
    setup_service_hosts_endpoint
    setup_key_value_endpoint
  end

  def setup_key_value_endpoint
    response = [
      {
        "CreateIndex" => 991,
        "ModifyIndex" => 11455806,
        "LockIndex" => 0,
        "Key" => "chef/services/#{service_name}",
        "Flags" => 0,
        "Value" => ""
      }
    ]
    stub_request(:get, "#{discovery_server}v1/kv/chef/services/#{service_name}").
        to_return(:status => 200, :body => response.to_json, :headers => {})
  end

  def setup_service_hosts_endpoint
    body_json = [
        {
            "Address" => "12.34.56.78",
            "ServicePort" => "5700",
            "Host" => "some-host-name"
        },
        {
            "Address" => "12.34.56.79",
            "ServicePort" => "2324",
            "Host" => "other-host-name"
        }
    ]
    stub_request(:get, "#{discovery_server}v1/catalog/service/#{service_name}").
        to_return(:status => 200, :body => body_json.to_json, :headers => {})
  end
end