module UAT
  module Consul
    class Configuration
      ...
      # @return [String] path that will be appended to consul host/port, no matter what service name
      def append_service_path
        @config_hash['append_service_path']
      end

      # @return [String] the prefix to prepend to a service name, used to retrieve consul key value requests for a service
      def service_metadata_key_value_prefix
        @config_hash['service_metadata_key_value_prefix']
      end

      # @return [String] the property name in consul key/value metadata hash containing a service's path
